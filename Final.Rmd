...
To prepare our data for this study, we took weighted average of each 
...
- PCpercent of households with no car and low access to stores in 2015
...
res2 <- lm(cbind(FOODINSEC_15_17, CH_FOODINSEC_14_17, VLFOODSEC_15_17) ~ PCT_LACCESS_POP15+PCT_OBESE_ADULTS17, data = df)
summary(res2)
...
for (i in 1:3){
  ...
  cat(paste0("95% simultaneous prediction interval for Y0",i, ":  (", pred_CI_Y), ")")
}
...
for (i in 1:3){
  ...
  pred_CI_Y <- c(t(a) %*%  cntr - half_CI, t(a) %*%  cntr + half_CI)
  cat(paste0("95% simultaneous confidence interval for Y0",i, ":  (", pred_CI_Y), ")\n")
}
```

Above we have our simultaneous confidence interval and simultaneous prediction interval. 
...
Above we have the factor analysis of our different factors of interest. In this case it was Racial Demographics, Health, Store Accesss, Access, and Food Insecurity. Looking at Food Insecurity, we see that
the total variance for this factors is somewhat explained by one factor, however, it is not very strong. Evaluating Store Access with the variables, Grocery store per thousand people, Super Center per thousand people, WICS-accepted stores per thousand people, Fast food restaurants per thousand people, Full-service restaurants and percent of SNAP redemptions in a county. We found this to by one our most significant loading with 78 percent of its variation being explained in this one factor.  Another similar factor was Access. Looking at our variables concering geographical access to a store near households, 77 percent of its variation can be explained by a single factor
...
```{r}
variables_to_test <-  c("PCT_LACCESS_POP15", "PCT_LACCESS_HHNV15","PCT_LACCESS_SNAP15","PCT_LACCESS_CHILD15","GROCPTH16","SUPERCPTH16","PCT_LACCESS_WHITE15","PCT_LACCESS_BLACK15","SNAPSPTH17","PCT_LACCESS_HISP15","PCT_LACCESS_NHASIAN15","PCT_LACCESS_NHNA15","PCT_LACCESS_NHPI15","FFRPTH16","FSRPTH16","PCT_SNAP17")
...
while (s <= S) {
  try({
    ...
    res_partial_model <- lm(cbind(FOODINSEC_15_17, CH_FOODINSEC_14_17, VLFOODSEC_15_17) ~ PCT_REDUCED_LUNCH15+PCT_OBESE_ADULTS17, data = bstp_df)
