...
Included in this method will be simultaneous confidence intervals for our predictor variables.
...
df<-df%>% 
  select(-FOODINSEC_12_14, -VLFOODSEC_12_14, -State, -CH_VLFOODSEC_14_17,-PCT_LACCESS_MULTIR15)
n <- nrow(df)
```
...
Y <- as.matrix(df %>% select(21:23))
m <- ncol(Y) # number of responses
Z <- as.matrix(cbind(const = 1, df %>% select(1:20)))
...
Y <- as.matrix(df %>% select(21:23))
m <- ncol(Y) # number of responses
Z <- as.matrix(cbind(const = 1, df %>% select(1:21)))
r <- ncol(Z) - 1  # number of predictors (excluding constant)
...
while (s <= S) {
  try({
    ...
    Y <- as.matrix(bstp_df %>% select(1:4))
    m <- ncol(Y)
    ...
    Z <- as.matrix(cbind(const = 1, bstp_df %>% select(1:21)))
    
    Z[ , grp2] <- Z[sample(nrow(Z)), grp2]   # shuffle rows of tested predictors
    
    r <- ncol(Z) - 1
    
    b <- solve(t(Z) %*% Z, tol = 2.05722e-17) %*% t(Z) %*% Y
    pred <- Z %*% b
    resid <- Y - pred
